...
_cache={}
def font(sz_pt, bold=False, italic=False):
    key=(round(sz_pt,1),bold,italic)
    if key not in _cache:
        _cache[key]=ImageFont.truetype(FONTS[(bold,italic)], int(round(ptpx(sz_pt))))
    return _cache[key]
...
def run_props(run):
    ...
    return dict(text=txt, size=sz, bold=bold, italic=italic, color=col, spc=spc)

def draw_text_run(draw, x, y, rp):
    f=font(rp['size'], rp['bold'], rp['italic'])
    if rp['spc']:
        step=ptpx(rp['spc'])
        for ch in rp['text']:
...
def render_shape(draw,img,sh):
    ...
    is_tri=False
    try: is_tri = (sh.auto_shape_type is not None and 'TRIANGLE' in str(sh.auto_shape_type))
    except Exception: pass
    if is_tri:
